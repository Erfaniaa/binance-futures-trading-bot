...
def parse_information(email_content):

    # ensure email_content is string not byte like
    email_content = email_content.decode('utf-8')

    # print(email_content)
    order_info_pattern = r'Your copied position from (.*) portfolio successfully opened at (.*)\(UTC\), order type: (\w+) , opening side: (\w+), open price: (\d+.\d+)'
    order_info_matches = re.search(order_info_pattern, email_content)

    if order_info_matches is None:
        return None

    portfolio_name, timestamp, order_type, opening_side, open_price = order_info_matches.groups()

    order_close_pattern = r'Your copied position from (.*) portfolio has been closed successfully at (.*)\(UTC\), order type: (\w+) , close price: (\d+.\d+), realized profit: (\d+.\d+)'
    order_close_matches = re.search(order_close_pattern, email_content)

    if order_close_matches is None:
        return {
    ...
    close_portfolio_name, close_timestamp, close_order_type, close_price, realized_profit = order_close_matches.groups()
    ...
    return {
        ...
        'opening_side': opening_side,
        'open_price': float(open_price),
        'close_portfolio_name': close_portfolio_name,
...
def read_emails_from_gmail():
    ...
    results = service.users().messages().list(userId='me',
                                              q="is:inbox is:unread subject:'Copied Position Successfully Opened'").execute()
    ...
    for message in messages:
        ...
        email_content = msg['payload']['parts'][0]['body']['data']
        finally_decoded = base64.urlsafe_b64decode(email_content)

        info = parse_information(finally_decoded)
    ...
    # print out all the info in a structured way
    print(info if info is not None else "No new Orders parsed.")
...
def update_current_time() -> int:
    ...
    current_time = datetime.fromtimestamp(
        binance_futures_api.time()["serverTime"] / 1000)
    return SUCCESSFUL
...
def get_m1_candles(
    ...
    number_of_kline_candles_requests = (end_timestamp - start_timestamp + MAXIMUM_KLINE_CANDLES_PER_REQUEST *
                                        ONE_MINUTE_IN_MILLISECONDS - 1) // (MAXIMUM_KLINE_CANDLES_PER_REQUEST * ONE_MINUTE_IN_MILLISECONDS)
    all_m1_candles_list = []
    for i in range(number_of_kline_candles_requests):
        current_time_range_start = i * MAXIMUM_KLINE_CANDLES_PER_REQUEST * \
            ONE_MINUTE_IN_MILLISECONDS + start_timestamp
        current_time_range_end = (i + 1) * MAXIMUM_KLINE_CANDLES_PER_REQUEST * \
            ONE_MINUTE_IN_MILLISECONDS + start_timestamp - 1
        current_time_range_start = max(
            current_time_range_start, start_timestamp)
        ...
        klines = retry(MAXIMUM_NUMBER_OF_API_CALL_TRIES, lambda e: logging.error(
            f"ERROR in get_m1_candles: {e}" or (ERROR, [])))(binance_futures_api.klines)
        current_time_range_m1_candles_list = klines(
            symbol=contract_symbol, interval="1m", startTime=current_time_range_start, endTime=current_time_range_end, limit=MAXIMUM_KLINE_CANDLES_PER_REQUEST)
...
def get_m15_candles(
    ...
    end_timestamp = int(end_datetime.timestamp() * 1000)
    number_of_kline_candles_requests = (end_timestamp - start_timestamp + MAXIMUM_KLINE_CANDLES_PER_REQUEST *
                                        ONE_MINUTE_IN_MILLISECONDS * 15 - 1) // (MAXIMUM_KLINE_CANDLES_PER_REQUEST * ONE_MINUTE_IN_MILLISECONDS * 15)
    all_m15_candles_list = []
    for i in range(number_of_kline_candles_requests):
        current_time_range_start = i * MAXIMUM_KLINE_CANDLES_PER_REQUEST * \
            ONE_MINUTE_IN_MILLISECONDS * 15 + start_timestamp
        current_time_range_end = (i + 1) * MAXIMUM_KLINE_CANDLES_PER_REQUEST * \
            ONE_MINUTE_IN_MILLISECONDS * 15 + start_timestamp - 1
        current_time_range_start = max(
            current_time_range_start, start_timestamp)
        current_time_range_start = min(current_time_range_start, end_timestamp)
        current_time_range_end = max(current_time_range_end, start_timestamp)
        ...
        klines = retry(MAXIMUM_NUMBER_OF_API_CALL_TRIES, lambda e: logging.error(
            f"ERROR in get_m15_candles: {e}" or (ERROR, [])))(binance_futures_api.klines)
        current_time_range_m15_candles_list = klines(
            symbol=contract_symbol, interval="15m", startTime=current_time_range_start, endTime=current_time_range_end, limit=MAXIMUM_KLINE_CANDLES_PER_REQUEST)
...
def get_h1_candles(
    ...
    end_timestamp = int(end_datetime.timestamp() * 1000)
    number_of_kline_candles_requests = (end_timestamp - start_timestamp + MAXIMUM_KLINE_CANDLES_PER_REQUEST *
                                        ONE_MINUTE_IN_MILLISECONDS * 60 - 1) // (MAXIMUM_KLINE_CANDLES_PER_REQUEST * ONE_MINUTE_IN_MILLISECONDS * 60)
    all_h1_candles_list = []
    for i in range(number_of_kline_candles_requests):
        current_time_range_start = i * MAXIMUM_KLINE_CANDLES_PER_REQUEST * \
            ONE_MINUTE_IN_MILLISECONDS * 60 + start_timestamp
        current_time_range_end = (i + 1) * MAXIMUM_KLINE_CANDLES_PER_REQUEST * \
            ONE_MINUTE_IN_MILLISECONDS * 60 + start_timestamp - 1
        current_time_range_start = max(
            current_time_range_start, start_timestamp)
        current_time_range_start = min(current_time_range_start, end_timestamp)
        current_time_range_end = max(current_time_range_end, start_timestamp)
        ...
        klines = retry(MAXIMUM_NUMBER_OF_API_CALL_TRIES, lambda e: logging.error(
            f"ERROR in get_h1_candles: {e}" or (ERROR, [])))(binance_futures_api.klines)
        current_time_range_h1_candles_list = klines(
            symbol=contract_symbol, interval="1h", startTime=current_time_range_start, endTime=current_time_range_end, limit=MAXIMUM_KLINE_CANDLES_PER_REQUEST)
...
def get_h2_candles(
    ...
    end_timestamp = int(end_datetime.timestamp() * 1000)
    number_of_kline_candles_requests = (end_timestamp - start_timestamp + MAXIMUM_KLINE_CANDLES_PER_REQUEST *
                                        ONE_MINUTE_IN_MILLISECONDS * 60 * 2 - 1) // (MAXIMUM_KLINE_CANDLES_PER_REQUEST * ONE_MINUTE_IN_MILLISECONDS * 60 * 2)
    all_h2_candles_list = []
    for i in range(number_of_kline_candles_requests):
        current_time_range_start = i * MAXIMUM_KLINE_CANDLES_PER_REQUEST * \
            ONE_MINUTE_IN_MILLISECONDS * 60 * 2 + start_timestamp
        current_time_range_end = (i + 1) * MAXIMUM_KLINE_CANDLES_PER_REQUEST * \
            ONE_MINUTE_IN_MILLISECONDS * 60 * 2 + start_timestamp - 1
        current_time_range_start = max(
            current_time_range_start, start_timestamp)
        current_time_range_start = min(current_time_range_start, end_timestamp)
        current_time_range_end = max(current_time_range_end, start_timestamp)
        ...
        klines = retry(MAXIMUM_NUMBER_OF_API_CALL_TRIES, lambda e: logging.error(
            f"ERROR in get_h2_candles: {e}" or (ERROR, [])))(binance_futures_api.klines)
        current_time_range_h2_candles_list = klines(
            symbol=contract_symbol, interval="2h", startTime=current_time_range_start, endTime=current_time_range_end, limit=MAXIMUM_KLINE_CANDLES_PER_REQUEST)
...
def get_h4_candles(
    ...
    end_timestamp = int(end_datetime.timestamp() * 1000)
    number_of_kline_candles_requests = (end_timestamp - start_timestamp + MAXIMUM_KLINE_CANDLES_PER_REQUEST *
                                        ONE_MINUTE_IN_MILLISECONDS * 60 * 4 - 1) // (MAXIMUM_KLINE_CANDLES_PER_REQUEST * ONE_MINUTE_IN_MILLISECONDS * 60 * 4)
    all_h4_candles_list = []
    for i in range(number_of_kline_candles_requests):
        current_time_range_start = i * MAXIMUM_KLINE_CANDLES_PER_REQUEST * \
            ONE_MINUTE_IN_MILLISECONDS * 60 * 4 + start_timestamp
        current_time_range_end = (i + 1) * MAXIMUM_KLINE_CANDLES_PER_REQUEST * \
            ONE_MINUTE_IN_MILLISECONDS * 60 * 4 + start_timestamp - 1
        current_time_range_start = max(
            current_time_range_start, start_timestamp)
        current_time_range_start = min(current_time_range_start, end_timestamp)
        current_time_range_end = max(current_time_range_end, start_timestamp)
        ...
        klines = retry(MAXIMUM_NUMBER_OF_API_CALL_TRIES, lambda e: logging.error(
            f"ERROR in get_h4_candles: {e}" or (ERROR, [])))(binance_futures_api.klines)
        current_time_range_h4_candles_list = klines(
            symbol=contract_symbol, interval="4h", startTime=current_time_range_start, endTime=current_time_range_end, limit=MAXIMUM_KLINE_CANDLES_PER_REQUEST)
...
def get_d1_candles(
    ...
    end_timestamp = int(end_datetime.timestamp() * 1000)
    number_of_kline_candles_requests = (end_timestamp - start_timestamp + MAXIMUM_KLINE_CANDLES_PER_REQUEST *
                                        ONE_MINUTE_IN_MILLISECONDS * 60 * 24 - 1) // (MAXIMUM_KLINE_CANDLES_PER_REQUEST * ONE_MINUTE_IN_MILLISECONDS * 60 * 24)
    all_d1_candles_list = []
    for i in range(number_of_kline_candles_requests):
        current_time_range_start = i * MAXIMUM_KLINE_CANDLES_PER_REQUEST * \
            ONE_MINUTE_IN_MILLISECONDS * 60 * 24 + start_timestamp
        current_time_range_end = (i + 1) * MAXIMUM_KLINE_CANDLES_PER_REQUEST * \
            ONE_MINUTE_IN_MILLISECONDS * 60 * 24 + start_timestamp - 1
        current_time_range_start = max(
            current_time_range_start, start_timestamp)
        current_time_range_start = min(current_time_range_start, end_timestamp)
        current_time_range_end = max(current_time_range_end, start_timestamp)
        ...
        klines = retry(MAXIMUM_NUMBER_OF_API_CALL_TRIES, lambda e: logging.error(
            f"ERROR in get_d1_candles: {e}" or (ERROR, [])))(binance_futures_api.klines)
        current_time_range_d1_candles_list = klines(
            symbol=contract_symbol, interval="1d", startTime=current_time_range_start, endTime=current_time_range_end, limit=MAXIMUM_KLINE_CANDLES_PER_REQUEST)
...
def update_recent_prices_list(
    ...
    if timeframe == "m1":
        ...
        recent_candles_list = get_m1_candles(contract_symbol, start_time, end_time)[
            1][:-1][-candles_count:]
        recent_m1_open_prices_list = [
            float(candle.open) for candle in recent_candles_list]
        recent_m1_high_prices_list = [
            float(candle.high) for candle in recent_candles_list]
        recent_m1_low_prices_list = [
            float(candle.low) for candle in recent_candles_list]
        recent_m1_close_prices_list = [
            float(candle.close) for candle in recent_candles_list]
    if timeframe == "m15":
        start_time = current_time - \
            timedelta(minutes=(candles_count + 10) * 15)
        end_time = current_time
        recent_candles_list = get_m15_candles(contract_symbol, start_time, end_time)[
            1][:-1][-candles_count:]
        recent_m15_open_prices_list = [
            float(candle.open) for candle in recent_candles_list]
        recent_m15_high_prices_list = [
            float(candle.high) for candle in recent_candles_list]
        recent_m15_low_prices_list = [
            float(candle.low) for candle in recent_candles_list]
        recent_m15_close_prices_list = [
            float(candle.close) for candle in recent_candles_list]
    if timeframe == "h1":
        start_time = current_time - \
            timedelta(minutes=(candles_count + 10) * 60)
        end_time = current_time
        recent_candles_list = get_h1_candles(contract_symbol, start_time, end_time)[
            1][:-1][-candles_count:]
        recent_h1_open_prices_list = [
            float(candle.open) for candle in recent_candles_list]
        recent_h1_high_prices_list = [
            float(candle.high) for candle in recent_candles_list]
        recent_h1_low_prices_list = [
            float(candle.low) for candle in recent_candles_list]
        recent_h1_close_prices_list = [
            float(candle.close) for candle in recent_candles_list]
    if timeframe == "h2":
        start_time = current_time - \
            timedelta(minutes=(candles_count + 10) * 60 * 2)
        end_time = current_time
        recent_candles_list = get_h2_candles(contract_symbol, start_time, end_time)[
            1][:-1][-candles_count:]
        recent_h2_open_prices_list = [
            float(candle.open) for candle in recent_candles_list]
        recent_h2_high_prices_list = [
            float(candle.high) for candle in recent_candles_list]
        recent_h2_low_prices_list = [
            float(candle.low) for candle in recent_candles_list]
        recent_h2_close_prices_list = [
            float(candle.close) for candle in recent_candles_list]
    if timeframe == "h4":
        start_time = current_time - \
            timedelta(minutes=(candles_count + 10) * 60 * 4)
        end_time = current_time
        recent_candles_list = get_h4_candles(contract_symbol, start_time, end_time)[
            1][:-1][-candles_count:]
        recent_h4_open_prices_list = [
            float(candle.open) for candle in recent_candles_list]
        recent_h4_high_prices_list = [
            float(candle.high) for candle in recent_candles_list]
        recent_h4_low_prices_list = [
            float(candle.low) for candle in recent_candles_list]
        recent_h4_close_prices_list = [
            float(candle.close) for candle in recent_candles_list]
    if timeframe == "d1":
        start_time = current_time - \
            timedelta(minutes=(candles_count + 10) * 60 * 24)
        end_time = current_time
        recent_candles_list = get_d1_candles(contract_symbol, start_time, end_time)[
            1][:-1][-candles_count:]
        recent_d1_open_prices_list = [
            float(candle.open) for candle in recent_candles_list]
        recent_d1_high_prices_list = [
            float(candle.high) for candle in recent_candles_list]
        recent_d1_low_prices_list = [
            float(candle.low) for candle in recent_candles_list]
        recent_d1_close_prices_list = [
            float(candle.close) for candle in recent_candles_list]
...
def update_indicators_dict(
    ...
    global indicators_dict
    if timeframe == "m1":
        candles_list = get_m1_candles(contract_symbol, datetime.fromtimestamp(
            indicators_dict["candle_close_timestamp"]), current_time)[1]
    if timeframe == "m15":
        candles_list = get_m15_candles(contract_symbol, datetime.fromtimestamp(
            indicators_dict["candle_close_timestamp"]), current_time)[1]
    if timeframe == "h1":
        candles_list = get_h1_candles(contract_symbol, datetime.fromtimestamp(
            indicators_dict["candle_close_timestamp"]), current_time)[1]
    ...
    for i in range(len(close_prices_list)):
        ...
        if timeframe == "h1" and current_time - timedelta(minutes=60) < datetime.fromtimestamp(_open_time):
            break
        _ema_50 = round(get_new_ema(_ema_50, _close_price, 50),
                        INDICATORS_DECIMAL_DIGITS)
        _ema_40 = round(get_new_ema(_ema_40, _close_price, 40),
                        INDICATORS_DECIMAL_DIGITS)
        _ema_30 = round(get_new_ema(_ema_30, _close_price, 30),
                        INDICATORS_DECIMAL_DIGITS)
        _ema_20 = round(get_new_ema(_ema_20, _close_price, 20),
                        INDICATORS_DECIMAL_DIGITS)
        _ema_10 = round(get_new_ema(_ema_10, _close_price, 10),
                        INDICATORS_DECIMAL_DIGITS)
        _macd_ema_12 = round(get_new_ema(
            _macd_ema_12, _close_price, 12), INDICATORS_DECIMAL_DIGITS)
        _macd_ema_26 = round(get_new_ema(
            _macd_ema_26, _close_price, 26), INDICATORS_DECIMAL_DIGITS)
        _macd_line = round(_macd_ema_12 - _macd_ema_26,
                           INDICATORS_DECIMAL_DIGITS)
        _signal_line = round(get_new_ema(
            _signal_line, _macd_line, 9), INDICATORS_DECIMAL_DIGITS)
        indicators_dict = {
...
def update_account_balance_and_unrealized_profit(first_coin_symbol: str) -> int:
    ...
    if len(last_account_available_balances_list) > LAST_ACCOUNT_BALANCES_LIST_MAX_LENGTH:
        last_account_available_balances_list = last_account_available_balances_list[
            -LAST_ACCOUNT_BALANCES_LIST_MAX_LENGTH:]
...
def update_contract_last_price(contract_symbol: str) -> int:
    ...
    contract_last_price = float(
        binance_futures_api.ticker_price(symbol=contract_symbol)["price"])
    return SUCCESSFUL
...
def close_all_open_positions_market_price() -> int:
    all_open_positions = binance_futures_api.get_position_risk(
    ...
    for position in all_open_positions:
        ...
        if position_quantity == 0.0:
            ...
            send_new_order_message(
                position["symbol"], "SELL", abs(position_quantity))
        elif position_quantity < 0.0:
            binance_futures_api.new_order(symbol=position["symbol"],
                                          side="BUY",
                                          quantity=abs(position_quantity),
                ...
                timestamp=get_local_timestamp())
            send_new_order_message(
                position["symbol"], "BUY", abs(position_quantity))
...
def init_bot() -> None:
    ...
    if IS_TESTNET:
        binance_futures_api = UMFutures(
            key=API_KEY_TESTNET, secret=SECRET_KEY_TESTNET, base_url=URL_BASE_TESTNET)
    else:
        binance_futures_api = UMFutures(
            key=API_KEY_PRODUCTION, secret=SECRET_KEY_PRODUCTION, base_url=URL_BASE_PRODUCTION)
...
def is_take_profit_unexecuted(contract_symbol: str, strategy_id: int) -> tuple:
    order_id = orders_dict.get(
        "strategy" + str(strategy_id) + "_last_take_profit_order_id", -1)
    if int(order_id) == -1:
        return (SUCCESSFUL, False)
    order = binance_futures_api.query_order(
        symbol=contract_symbol, orderId=order_id, timestamp=get_local_timestamp())
...
def is_stop_loss_unexecuted(contract_symbol: str, strategy_id: int) -> tuple:
    order_id = orders_dict.get(
        "strategy" + str(strategy_id) + "_last_stop_loss_order_id", -1)
    if int(order_id) == -1:
        return (SUCCESSFUL, False)
    order = binance_futures_api.query_order(
        symbol=contract_symbol, orderId=order_id, timestamp=get_local_timestamp())
...
def cancel_extra_open_order(contract_symbol: str, strategy_id: int) -> int:
    ...
    if is_take_profit_unexecuted(contract_symbol, strategy_id)[1] and not \
        ...
        order_id = orders_dict["strategy" +
                               str(strategy_id) + "_last_take_profit_order_id"]
    elif not is_take_profit_unexecuted(contract_symbol, strategy_id)[1] and \
            is_stop_loss_unexecuted(contract_symbol, strategy_id)[1]:
        order_id = orders_dict["strategy" +
                               str(strategy_id) + "_last_stop_loss_order_id"]

    if order_id is not None:
        binance_futures_api.cancel_order(
            symbol=contract_symbol, orderId=order_id, timestamp=get_local_timestamp())
...
def set_leverage(contract_symbol: str, leverage: int) -> int:
    binance_futures_api.change_leverage(
        symbol=contract_symbol, leverage=leverage, timestamp=get_local_timestamp())
    return SUCCESSFUL
...
def set_position_mode(hedge_mode: bool) -> int:
    binance_futures_api.change_position_mode(
        dualSidePosition="true" if hedge_mode else "false", timestamp=get_local_timestamp())
    return SUCCESSFUL
...
def open_long_position(
    ...
    position_quantity = round_down(WALLET_USAGE_PERCENT / 100 / STRATEGIES_COUNT *
                                   first_coin_amount / contract_last_price, POSITION_QUANTITY_DECIMAL_DIGITS)
    ...
    for i in range(MAXIMUM_NUMBER_OF_API_CALL_TRIES):
        try:
            ...
            market_order = binance_futures_api.query_order(
                symbol=contract_symbol, orderId=market_order["orderId"], timestamp=get_local_timestamp())
            try:
            ...
            take_profit_price = round(
                (1 + take_profit_percent / 100) * position_entry_price, PRICE_DECIMAL_DIGITS)
            stop_loss_price = round(
                (1 + stop_loss_percent / 100) * position_entry_price, PRICE_DECIMAL_DIGITS)
            take_profit_price_limit = round(
                take_profit_price - 10 ** (-PRICE_DECIMAL_DIGITS), PRICE_DECIMAL_DIGITS)
            stop_loss_price_limit = round(
                stop_loss_price - 10 ** (-PRICE_DECIMAL_DIGITS), PRICE_DECIMAL_DIGITS)
            ...
            send_open_long_position_message(
                "strategy" + str(strategy_id) + "_last_take_profit_order_id")
            ...
            send_new_order_message(contract_symbol, "SELL", position_quantity)
            send_open_long_position_message(
                "strategy" + str(strategy_id) + "_last_stop_loss_order_id")
            update_orders_dict(get_local_timestamp(), "strategy" + str(strategy_id) +
                               "_last_take_profit_order_id", take_profit_order["orderId"])
            update_orders_dict(get_local_timestamp(), "strategy" + str(strategy_id) +
                               "_last_stop_loss_order_id", stop_loss_order["orderId"])
    ...
    for i in range(MAXIMUM_NUMBER_OF_API_CALL_TRIES):
        try:
            ...
            take_profit_order = binance_futures_api.new_order(symbol=contract_symbol,
                                                              ...
                                                              timestamp=get_local_timestamp())
            send_new_order_message(contract_symbol, "BUY", position_quantity)
            send_open_short_position_message(
                "strategy" + str(strategy_id) + "_last_take_profit_order_id")
            stop_loss_order = binance_futures_api.new_order(symbol=contract_symbol,
                                                            side="BUY",
                                                            positionSide="SHORT",
...
def main() -> None:
    ...
    while True:
        ...
        update_current_time()
        update_recent_prices_list(
            CONTRACT_SYMBOL, current_time, IMPORTANT_CANDLES_COUNT, TIMEFRAME)
        save_indicators_dict()
        update_contract_last_price(CONTRACT_SYMBOL)
        update_is_price_increasing(
            PRICE_DIRECTION_INDICATOR_NAME_1, PRICE_DIRECTION_INDICATOR_NAME_2)
        update_is_price_decreasing(
            PRICE_DIRECTION_INDICATOR_NAME_1, PRICE_DIRECTION_INDICATOR_NAME_2)
